...
function drawCursorRibbon() {
  ...
  gradient.addColorStop(0.45, cursorHover
...
function updateScrollProgress() {
  ...
  scrollTopButton?.classList.toggle("visible", scrollY > innerHeight * 0.6);
}
...
function setupMobileMenu() {
  ...
  menuToggle.addEventListener("click", () => {
    const willOpen = !navPill.classList.contains("open");

    menuToggle.classList.toggle("active", willOpen);
    navPill.classList.toggle("open", willOpen);
...
function setupContactForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    status.textContent =
      "Message ready. Opening mail client...";
...
function setupCursorInteractions() {
  ...
  document.querySelectorAll(
    "a, button, input, textarea, [data-tilt]"
  ).forEach((el) => {
